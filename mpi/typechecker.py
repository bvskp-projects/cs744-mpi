...
from mpi.types import Attrs, Callable, ClassType, SemError, SynError
...
class TypeCheckerPass(ast.NodeVisitor):
    ...
    def visit_Call(self, call):
        ...
        formal_args = [
            arg for arg in reversed(func_sig.args)
        ]  # reversed to pop args from behind
        if len(formal_args) < len(call.args):
            raise SemError(
                f"Function called with invalid number of arguments! lineno:{lno}"
            )
        for actual_arg in call.args:
            ...
            if not is_consistent_with(actual_attrs, formal_args[-1].attrs):
                ...
                raise SemError(
                    f"Function called with invalid arg {aname}! lineno:{lno}"
                )
        ...
        for keyword in call.keywords:
            ...
            if aname not in formal_args:
                raise SemError(
                    f"keyword not available in remaining arguments! lineno:{lno}"
                )
        ...
        if call_attrs is LayerAttrs["__init__"]:
            if not isinstance(call.args[0], ast.Name) or not isinstance(
    ...
    def get_graph_local_name(self, subscript):
        ...
        if graph_attrs is NodeDataAttrs:
            ...
            raise SemError(
                f"Generic subscripts are not supported! lineno:{subscript.lineno}"
            )
    ...
    def visit_Assign(self, assign):
        ...
        if formal_attrs is not None:
            ...
            if not is_consistent_with(value_attrs, formal_attrs):
                raise SemError(
                    f"Type mismatch: cannot assign to variable! lineno:{lno}"
                )
        elif isinstance(target, ast.Name):
            # register a new local variable
            if target.id.endswith("_"):
                raise SynError(
                    f"Variables cannot have a trailing underscore! lineno:{lno}"
                )
            ...
            if not self.init_mode:
                raise SemError(
                    "Cannot register new instance variables outside of __init__!"
                    f" lineno:{lno}"
                )
    ...
    def visit_AnnAssign(self, assign):
        if self.return_attrs:
            raise SemError(
                "Annotated assignments are not supported in func body!"
    ...
    def visit_ClassDef(self, classdef):
        ...
        if (
            "__init__" not in self_attrs
            or "reset_parameters" not in self_attrs
            or "forward" not in self_attrs
        ):
            raise SemError(
                f"Must define __init__, reset_parameters, and forward! lineno:{lno}"
            )
        ...
        if len(reset_func.args) != 0 or not is_consistent_with(
            reset_func.return_attrs, NoneAttrs
        ):
