...
from mpi.attrs import GlobalAttrsPass, InstanceAttrsPass
from mpi.builtins import get_builtin_typemap
from mpi.filter import FeatureFilterPass
from mpi.layerpass import run_layer_pass
from mpi.render import CodeRenderer
from mpi.symtab import SymbolTable
from mpi.types import SemError
from mpi.utils import camel_to_snake
...
class LayerExtractorPass(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, classdef):
        ...
        if len(classdef.bases) != 1 or ast.unparse(classdef.bases[0]) != "mpi.Module":
            raise SemError(
                f"All classes must inherit from mpi.Module! lineno:{classdef.lineno}"
            )
