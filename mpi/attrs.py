...
from mpi.builtins import IntAttrs, NoneAttrs, get_builtin_classes
from mpi.symtab import SymbolTable
from mpi.types import Arg, Attrs, Callable, ClassType, SemError, SynError
...
class GlobalAttrsPass(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, classdef):
        if classdef.name in self.global_attrs:
            raise SemError(
                f"Class {classdef.name} is multiply defined! lineno:{classdef.lineno}"
            )
        ...
        self.classes[class_type] = Attrs(
            {"_mpic_class": class_type, "input_dim": IntAttrs, "output_dim": IntAttrs}
        )
    ...
    def visit_Module(self, module):
        for child in module.body:
            if isinstance(child, ast.ClassDef):
                ...
                raise SemError(
                    "Only mpi.Module classes allowed at the top level!"
                    f" lineno:{child.lineno}"
                )
...
class InstanceAttrsPass(ast.NodeVisitor):
    ...
    def visit_AnnAssign(self, assign):
        ...
        if not assign.annotation:
            raise SynError(
                f"Require type annotation for instance variables! lineno:{lno}"
            )
    ...
    def visit_FunctionDef(self, func):
        ...
        if func.name == "reset":
            raise SynError(
                f"Please use reset_parameters to reset torch params! lineno:{lno}"
            )
    ...
    def visit_ClassDef(self, classdef):
        ...
        if classdef.decorator_list:
            raise SemError(
                f"Decorator lists are not supported! lineno:{classdef.lineno}"
            )

        if classdef.keywords:
            raise SemError(
                f"Class arguments are not supported! lineno:{classdef.lineno}"
            )
