...
from mpi.builtins import DENSEGraphAttrs, LinearAttrs, builtin_attrs, node_data_prefix
from mpi.types import Attrs, Callable, ClassType, SemError

instance_var_suffix = "_"


def extract_options(
    classdef: ast.ClassDef, class_attrs: Attrs, typemap: dict[str, str]
...
def extract_member_variables(
    class_attrs: Attrs, typemap: dict[str, str]
) -> dict[str, str]:
    member_vars = dict()
    for name, attrs in class_attrs.items():
        if (
            name != "_mpic_class"
            and name not in {"input_dim", "output_dim"}
            and isinstance(attrs, Attrs)
        ):
...
def extract_func_decls(class_attrs: Attrs, typemap: dict[str, str]) -> list[dict]:
    ...
    for name, attrs in class_attrs.items():
        if (
            ...
            args = [
                f"{arg_type} {name}" for arg_type, name in zip(arg_types, arg_names)
            ]
...
def extract_local_vars(
    func_vars: dict[str, dict[str, Attrs]], typemap: dict[str, str]
) -> dict[str, dict[str, str]]:
    return {
        func_name: {
            name: typemap[attrs["_mpic_class"]] for name, attrs in local_vars.items()
        }
...
class FunctionGenerator(ast.NodeVisitor):
    ...
    def visit_Call(self, call):
        ...
        keyword_args = [
            keywords[arg.name] for arg in func_attrs.args[len(pos_args) :]  # noqa: E203
        ]
        ...
        if func_attrs is DENSEGraphAttrs["update_all"]:
            ...
            register = f'register_parameter("{target}", {init_tensor})'
            return f"{target} = {autograd};"
        elif func_attrs is LinearAttrs["__call__"]:
            target = self.visit(call.func)
            inputs = self.visit(call.args[0])
            return (
                f"torch::matmul({target}, {inputs}.transpose(0, -1)).transpose(0, -1)"
            )
        else:
            args_expr = ", ".join(args)
